...
class MainActivity : AppCompatActivity() {
    ...
    private fun setSpinner() {
        ...
        val colors = listOf("#D32F2F", "#eb5a4f", "#ebcd4f", "#aa5a4f")
        val labels = listOf("Ayam", "Soto", "Sate", "Lele")
        ...
        for (i in 1..colorCount) {
            ...
            spinWheelItemSectionModel.secondaryText = labels[i-1]
            spinWheelItemSectionModel.color = colors[i-1].toColorInt()
        ...
        luckySpinnerView.setLuckyRoundItemSelectedListener { index ->
            Toast.makeText(
                applicationContext,
                "You got ${data[index].secondaryText}",
                Toast.LENGTH_SHORT
            ).show()
        }
    }
